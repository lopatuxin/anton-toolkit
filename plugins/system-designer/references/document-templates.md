...
**Folder layout:** each roadmap lives in its own subfolder `docs/roadmaps/<slug>/roadmap.md`, together with its phase documents in `docs/roadmaps/<slug>/phases/`. Roadmaps NEVER go directly into `docs/` (no `docs/roadmap.md`, no `docs/roadmap-<slug>.md`). The slug is kebab-case, derived from the scope (e.g. `auth`, `public`, `mvp`). For a project with a single overall roadmap, default to slug `main`.
...
## roadmaps/<slug>/phases/phase-NN-<phase-slug>.md

**Folder layout:** phase documents live next to their roadmap, in `docs/roadmaps/<slug>/phases/`, NEVER in a top-level `docs/phases/` folder. Each roadmap owns its own `phases/` subdirectory so that phases of different scopes (auth vs public vs payments) do not mix.
...
- Имя файла — `phase-NN-<phase-slug>.md`, где `NN` с ведущим нулём (`phase-01-registration.md`), `phase-slug` — kebab-case из названия фазы. Полный путь — `docs/roadmaps/<roadmap-slug>/phases/phase-NN-<phase-slug>.md`.
