...
- All files under `docs/`: `concept.md`, `architecture.md`, `modules/*.md`, `roadmaps/*/roadmap.md` (если существуют — их может быть несколько по разным срезам системы), `roadmaps/*/phases/*.md` (если существуют). Legacy-пути `docs/roadmap.md`, `docs/roadmap-*.md`, `docs/phases/*.md` тоже сканируй — если встретишь, отметь их в репорте как «требует миграции в docs/roadmaps/<slug>/», но не переноси сам без указания оркестратора.
...
2. **Classify the change.** Which of these is it:
   ...
   - **Roadmap / phases affected**: если изменение меняет состав или порядок фаз — правь соответствующий `docs/roadmaps/<slug>/roadmap.md` (определи затронутый slug по контексту изменения — например auth-related правки идут в `roadmaps/auth/`). Если изменение меняет scope, интерфейсы или модели данных, используемые в уже детализированных `docs/roadmaps/<slug>/phases/*.md` — обнови затронутые phase-документы в той же папке. Если из-за изменения появляется / удаляется / переименовывается фаза — приведи `roadmap.md` и `phases/` соответствующего slug в соответствие (создай новый phase-файл, удали устаревший, переименуй). Новые phase-файлы создавай строго в `docs/roadmaps/<slug>/phases/`, никогда в legacy-папке `docs/phases/`.
