...
- All files under `docs/`: `concept.md`, `architecture.md`, `modules/*.md`, `roadmaps/*/roadmap.md` (if present — there may be several, one per system slice), `roadmaps/*/phases/*.md` (if present). Also scan legacy paths `docs/roadmap.md`, `docs/roadmap-*.md`, `docs/phases/*.md` — if you find them, flag them in the report as "requires migration to docs/roadmaps/<slug>/", but do NOT move them yourself without explicit orchestrator instruction.
...
2. **Classify the change.** Which of these is it:
   ...
   - **Roadmap / phases affected**: если изменение меняет состав или порядок фаз — правь соответствующий `docs/roadmaps/<slug>/roadmap.md` (determine the affected slug from the change context — e.g. auth-related edits go into `roadmaps/auth/`). Если изменение меняет scope, интерфейсы или модели данных, используемые в уже детализированных `docs/roadmaps/<slug>/phases/*.md` — обнови затронутые phase-документы в той же папке. Если из-за изменения появляется / удаляется / переименовывается фаза — приведи `roadmap.md` и `phases/` соответствующего slug в соответствие (создай новый phase-файл, удали устаревший, переименуй). Always create new phase files strictly under `docs/roadmaps/<slug>/phases/` — never in the legacy `docs/phases/` folder.
