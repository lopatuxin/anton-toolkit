...
Read these files from `C:\projects\obsidian\Проекты\youtube\база\`:
...
Glob `C:\projects\obsidian\Проекты\youtube\конкуренты\*.md`. Take the 2 most recent files by date in the filename prefix (`YYYY-MM-DD-*.md`).
...
- **Evidence (specific):** <cite KB slug or competitor report line — e.g.,
  "[[Проекты/youtube/база/видео/курсор-выкинул-проект]] — top, vpd 2.4x median, transcript
  shows hook pattern X" or "[[Проекты/youtube/конкуренты/2026-05-12-разбор-ниши]] §8 gap:
...
- Target folder: `C:\projects\obsidian\Проекты\youtube\идеи\` (same folder as `yt-ideas`, different `type` in frontmatter).
...
- Frontmatter:
  ...
  kb_fresh: <true | false>
  competitor_inputs:
    - "[[Проекты/youtube/конкуренты/<file-without-ext>]]"
  tags:
    - youtube/strategic-shortlist
  related:
    - "[[Проекты/youtube/база/канал]]"
    - "[[Проекты/youtube/база/индекс]]"
  ---
  ```
