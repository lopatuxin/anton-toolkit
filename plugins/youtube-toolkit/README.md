...
All artefacts live in the Obsidian vault at `C:\projects\obsidian\Проекты\youtube\`. See `references/vault.md` for folder layout, filename rules (Cyrillic kebab-case), and frontmatter conventions.
...
Skills write Markdown to `C:\projects\obsidian\Проекты\youtube\`. Folders (one per tool, Cyrillic, lowercase):
