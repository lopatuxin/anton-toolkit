...
1. **Read the analytical input.** Pull out:
   ...
   - From the knowledge base: titles + transcripts of top cards (signal for what hooks/topics worked), titles + `classification_reason` of fail cards (signal for what to avoid). When citing in the "Why this idea" field, reference the specific video by slug, e.g. `"KB: [[Проекты/youtube/база/видео/курсор-выкинул-проект]] — top, vpd 2.4x median"`.
...
- Target folder: `C:\projects\obsidian\Проекты\youtube\идеи\`.
...
- Frontmatter:
  ...
  tags:
    - youtube/idea-batch
  related:
    - "[[Проекты/youtube/канал/<channel-file>]]"          # if a yt-my-channel report was used
    - "[[Проекты/youtube/конкуренты/<competitor-file>]]"  # if a yt-competitors report was used
  ---
  ```
