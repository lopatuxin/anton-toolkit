...
6. **Persist the report to the Obsidian vault.** Follow `${CLAUDE_PLUGIN_ROOT}/references/vault.md`. Target folder: `C:\projects\obsidian\Проекты\youtube\канал\`. Filename: `YYYY-MM-DD-обзор-канала.md` (Russian, Cyrillic, kebab-case — see `${CLAUDE_PLUGIN_ROOT}/references/vault.md` for the filename rules) (use the current session date; if a snapshot for today already exists, append `-v2`, `-v3`…). Frontmatter:
