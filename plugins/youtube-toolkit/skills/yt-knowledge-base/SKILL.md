...
```
C:\projects\obsidian\Проекты\youtube\база\
...
Check whether `C:\projects\obsidian\Проекты\youtube\база\канал.md` exists:
- Exists → **REFRESH** mode.
...
For each video, write `C:\projects\obsidian\Проекты\youtube\база\видео\<slug>.md` using the template in `${CLAUDE_SKILL_DIR}/references/video-card-template.md`. Use the SAME slug rules as the rest of the vault (Cyrillic, lowercase, kebab-case, derived from working title — see `${CLAUDE_PLUGIN_ROOT}/references/vault.md`).
...
- **Output:** files under `C:\projects\obsidian\Проекты\youtube\база\` + one short Russian summary in chat.
...
- Do NOT write outside `C:\projects\obsidian\Проекты\youtube\база\`.
