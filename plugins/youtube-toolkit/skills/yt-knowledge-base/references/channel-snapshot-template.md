...
One file at `C:\projects\obsidian\Проекты\youtube\база\канал.md`. Overwritten on every run (this file is the "latest state" snapshot, not a history). Per-snapshot dated reports live in `канал\` (the `yt-my-channel` folder) — do NOT confuse the two.
