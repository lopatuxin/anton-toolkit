...
description: >
  ...
  experiment, or a dead end as its own searchable note; review entries that are awaiting the user's
  evaluation; search the journal by area / type / weight / status / date instead of scrolling through
  it; and mark the outcome of an experiment (worked / failed), adjusting its importance weight. Each
  ...
  "покажи решения по памяти", "покажи тупики logos", "что на ревью logos",
  ...
  for designing the Logos architecture use logos-design. This skill only records, reviews, searches,
...
- **REVIEW** — "что на ревью", "ревью журнала", "пройдёмся по нерассмотренным".
...
3. Set initial frontmatter per `references/diary-format.md` section 4: today's `дата`, the `тип` and
   `область`, `вес: 5` (default), `статус` (`предложено` for a decision, `проверяется` for an
   experiment, `тупик`/`откат` for those), `ревью: false`.
...
5. Confirm in Russian, one line: «Записал в журнал: `Logos/Журнал/<имя>.md` (тип: <тип>, область: <область>, вес: 5, ждёт ревью).»

## 3. REVIEW mode

1. Find entries with `ревью: false`. Prefer reading the frontmatter of files in `$VAULT/Logos/Журнал/`
   (grep `ревью: false`); these are exactly what the folder note's "На ревью" Dataview query shows.
2. Present them to the user one at a time in Russian, briefly (title + тип + область + the decision).
3. For each, ask the user for: accept/reject (or outcome), an importance `вес` (1–10), and any
   feedback. Then update that entry's frontmatter (`статус`, `вес`, `ревью: true`) and fill the
   `## Ревью` section with their verbatim feedback. Never overwrite unrelated content.
4. After the batch, confirm in Russian how many entries were reviewed.
...
2. **Point to the live index:** remind the user that `Logos/Журнал/Журнал.md` has live Dataview
   tables (На ревью / По важности / Тупики и провалы / Все записи) they can open in Obsidian for an
...
- **Never overwrite a different entry.** Re-recording the same decision updates ITS file (status,
  result, review); a new decision is a new file.
