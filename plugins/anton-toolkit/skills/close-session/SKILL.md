...
- Before writing `next`, check which tasks are already merged — never name a merged task as in progress. In the card's `repo:` run `git fetch --prune`, then `git branch -r` and `git branch -vv`. A task is merged when its branch (the branch name carries the task ID, e.g. `feature/TRADEX-543/backoffice_roles`) is gone from the remote: the MR deletes the branch on merge, and `git branch -vv` marks the local copy `[origin/...: gone]`. Pushed is not merged: a branch still on the remote is in review or in work. A local branch that was never pushed (no upstream) is in work, not merged. Real case: the otc_desk card said «В работе TRADEX-518, 543, 562» while all three branches were already gone from the remote.
