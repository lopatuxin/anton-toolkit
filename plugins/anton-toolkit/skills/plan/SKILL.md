...
- **Не перекладывай вычисления на фронтенд**: если в проекте есть бэкенд-сервис, производные метрики (суммы, средние, агрегаты) должны вычисляться в backend (Service/DTO) и возвращаться через API. Фронтенд только отображает готовые данные. Нарушение: вычислять `totalYear = reduce(...)` или `average = sum / 12` прямо в React-компоненте вместо добавления полей в DTO.
