...
4. **Если коллекции НЕТ** — создай её:
   - Создай коллекцию с именем по сервису/фиче (например "Orders API Tests")
   - Добавь запросы для каждого эндпоинта через `create_request`: happy path, валидация, граничные случаи
   - Коллекция останется в Postman для будущих тестов и регресса
...
### 7. Очисти временные файлы (ОБЯЗАТЕЛЬНО)

**Этот шаг выполняется ВСЕГДА, даже если тесты упали.** Отчёт НЕ считается завершённым пока файлы не удалены.

**КРИТИЧЕСКИ ВАЖНО:** Скриншоты Playwright сохраняются В КОРЕНЬ ПРОЕКТА (cwd), а не в текущую папку bash. Используй абсолютные пути или `find` для гарантированной очистки:

```bash
# Найди и удали ВСЕ .png/.jpg в корне проекта (не рекурсивно, чтобы не удалить ассеты)
...
**Проверь результат** — выполни `ls *.png *.jpg .playwright-mcp 2>/dev/null` и убедись что вывод пустой. Если файлы остались — удали их повторно.
