...
- A hand-made mechanism where a standard one exists. For every new infrastructure class ask: does Spring, Boot or a library already do this? A custom `EnvironmentPostProcessor` for shared defaults (vs `spring.config.import`), a hand-made limiter or retry (vs resilience4j), a table mapping class names held as strings to behaviour (vs an annotation attribute on the method) are Warnings. So is a base class kept alive by flags, default-`null` hooks and throwing stubs for one subclass, a helper with one method per combination of steps, and a new `@Qualifier` outside the several-connections-of-one-type case — the Java conventions list the expected shapes.
